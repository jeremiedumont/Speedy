...
public class GameIntense extends GameSuperClass implements InputProcessor, Screen {

    Label lab2, lab3, labChar;
    float coeffSpeedMove=0.3f;
    int angle1=100, angle2=10, angle3=50;
    String charD2="A", charD3="A", charA2="D", charA3="D", strA;
    ...
    public void show() {
        init(GlobalVariables.gameEnum.gameIntense);
        ...
        lab3= new Label("8", new Label.LabelStyle(font, Color.BLACK));
        lab3.setOrigin(lab1.getWidth()/2, lab3.getHeight()/2);
        lab3.setPosition(Gdx.graphics.getWidth()/2-lab3.getWidth()/2, Gdx.graphics.getHeight()/2-lab3.getHeight()/2);

        labChar = new Label(strA, skin2);
        labChar.setFontScale(Gdx.graphics.getWidth()/300.0f);
        labChar.setPosition(Gdx.graphics.getWidth()/6.5f, Gdx.graphics.getHeight()/numH*8.8f);
        ...
        temp=(rd.nextInt(9)+49);
        charA3=Character.toString((char)temp);
        strA = charA1+"--"+charA2+"--"+charA3;
    ...
    public void render(float delta) {
        ...
        lab3.setPosition((float)(lab3.getX()+Gdx.graphics.getDeltaTime()*Gdx.graphics.getHeight()*coeffSpeedMove*Math.cos(Math.toRadians(angle3))), (float)(lab3.getY()+Gdx.graphics.getDeltaTime()*Gdx.graphics.getHeight()*coeffSpeedMove*Math.sin(Math.toRadians(angle3))));
        lab1.draw(batch, 1f);
        lab2.draw(batch, 1f);
        lab3.draw(batch, 1f);
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        if(screenY<Gdx.graphics.getHeight()/numH*8 && screenY>Gdx.graphics.getHeight()/numH*2) { //Si on se situe dans le cadre de jeu
            ...
            if ((charA1.equals(charD1) && charA2.equals(charD2) && charA3.equals(charD3)) || (charA1.equals(charD1) && charA2.equals(charD3) && charA3.equals(charD2)) || (charA1.equals(charD3) && charA2.equals(charD2) && charA3.equals(charD1)) || (charA1.equals(charD3) && charA2.equals(charD1) && charA3.equals(charD2)) || (charA1.equals(charD2) && charA2.equals(charD1) && charA3.equals(charD3)) || (charA1.equals(charD2) && charA2.equals(charD3) && charA3.equals(charD1))) { //Si le caractère appuyé est le bon
                do {
                    temp = (rd.nextInt(9) + 49);
                ...
                do {
                    temp = (rd.nextInt(9) + 49);
                } while (charA3.equals(Character.toString((char) temp)));
                charA3 = Character.toString((char) temp); //On change le caractère recherché
                strA = charA1+"--"+charA2+"--"+charA3;
                generateAngle();
                coeffSpeedMove+=0.025;
            ...
            if(GlobalVariables.actualLife==0) {
                ...
                GameOverWindow wd = new GameOverWindow("", new Skin(Gdx.files.internal("skin_glassy/glassy-ui.json")), score, stageGameOver, this, GlobalVariables.gameEnum.gameIntense);
    ...
    public void newCharacter()
    {
        if(rd.nextInt(5)==1){
            ...
            temp=(rd.nextInt(9)+49);
            charD3=Character.toString((char)temp);
        }
        lab1.setText(charD1);
        lab2.setText(charD2);
        lab3.setText(charD3);
    ...
    public void reboundsLabel(){
        ...
        if(lab3.getX()<=0){
            angle3=180-angle3;
            lab3.setX(lab3.getX()+5);
        }
        else if(lab3.getX()+lab3.getWidth()>=Gdx.graphics.getWidth()){
            angle3=180-angle3;
            lab3.setX(lab3.getX()-5);
        }
        else if(lab3.getY()<=Gdx.graphics.getHeight()/numH*2){
            angle3=360-angle3;
            lab3.setY(lab3.getY()+5);
        }
        else if(lab3.getY()+lab3.getHeight()>=Gdx.graphics.getHeight()/numH*8){
            angle3=360-angle3;
            lab3.setY(lab3.getY()-5);
        }
    }
    ...
    public void generateAngle(){
        do{
            ...
            angle3=rd.nextInt(359);
        }while(Math.abs(angle1-angle2)<=45 || Math.abs(angle3-angle2)<=45 || Math.abs(angle1-angle3)<=45);
    }
